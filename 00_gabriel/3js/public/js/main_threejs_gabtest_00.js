...
function getSphere(size, material){
        ...
        var materialBasic = new THREE.MeshBasicMaterial ( {color: 'rgb(255,255,255)'
      });
      var materialPhong = new THREE.MeshPhongMaterial ( {color: 'rgb(120,255,100)'
    });
        var matBasic = materialBasic;
        var matPhong = materialPhong;


        var mesh = new THREE.Mesh( geometry, material);
        if(materialPhong = true){
          mesh.castShadow = true;
        }
//forgot to add "return mesh". I was just stating the var, but not running it. Thought that "add.box" would be enough.
        return mesh;
